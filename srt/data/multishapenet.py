...
class MultishapenetDataset(IterableDataset):
    ...
    #     #
    #     scale_factor = 0.25
    #     # print(data['color_image'].shape, data['ray_directions'].shape, data['ray_origins'].shape, target_views.shape)
    #
    #     data['color_image'] = F.interpolate(torch.Tensor(data['color_image']).permute(0,3,1,2), scale_factor=scale_factor, mode='nearest').permute(0,2,3,1).numpy()
    #     data['ray_directions'] = F.interpolate(torch.Tensor(data['ray_directions']).permute(0,3,1,2), scale_factor=scale_factor, mode='nearest').permute(0,2,3,1).numpy()
    #     data['ray_origins'] = F.interpolate(torch.Tensor(data['ray_origins']).permute(0,3,1,2),  scale_factor=scale_factor, mode='nearest').permute(0,2,3,1).numpy()
    #
    #     # print(data['color_image'].shape, data['ray_directions'].shape, data['ray_origins'].shape, target_views.shape)
    #
    ...
    #
    #     target_images = np.transpose(data['color_image'][target_views], (0, 3, 1, 2))
    ...
    #     target_camera_pos = data['ray_origins'][target_views][:, 0, 0]
    #
    #
    #
